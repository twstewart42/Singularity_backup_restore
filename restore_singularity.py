...
def parsejson(blob, fileo):
	#converts string of blob to Python dictionarys and reassembles them to individual curl requests
	...
	print req_data
	curlrequest = 'curl -i -X POST -H \'Content-Type: application/json\' -d \'%s\' app001:8082/singularity/api/requests' % req_data
        print curlrequest
	...
	try:
		...
		curldeploy = 'curl -i -X POST -H \'Content-Type: application/json\' -d \'%s\' app001:8082/singularity/api/deploys' % string_merged
	        print curldeploy
	except:
		print "no deploy"
		curldeploy = ""
...
def getcount():
	...
	db=MySQLdb.connect(host="app001.example.com", user="singularity", passwd="mesos", db="singularity")
	q=db.cursor()
...
def main():
	...
	#blob = '{"request":{"id":"QA_RTMA","requestType":"SCHEDULED","owners":["me@example.com"],"schedule":"54 * * * *","quartzSchedule":"0 54 * * * ?"},"state":"ACTIVE","requestDeployState":{"requestId":"TASK1","activeDeploy":{"requestId":"TASK1","deployId":"new_TASK1","timestamp":1463933957851}},"activeDeploy":{"requestId":TASK1","id":"new_TASK1","command":"/usr/local/bin/task1.cpp","resources":{"cpus":1.0,"memoryMb":512.0,"numPorts":0}}}'
	num = getcount()
	...
	for c in range(0, num):
		...
		while True:
			...
			try:
				...
				#This craziness is because some of the JSON have extra double quotes that need to be a part of the JSON file but python does not like that so we have to manually add escapes for them in
