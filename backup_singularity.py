#!/usr/bin/env python
##########################################################
#	Makes a backup of the weather singularity/mesos cluster request and deploy configs
#	- T. Stewart
##############################################################
import subprocess
import os
...
#change fake hostnames to real ones in your env

def exportCSV():
	#exports full backup table to csv
	YMd = datetime.datetime.now().strftime('%Y-%m-%d')
	db2=MySQLdb.connect(host="app001.example.com", user="singularity", passwd="mesos", db="singularity")
        da=db2.cursor()
...
def get_requests():
	...
	request = "/usr/lib/zookeeper/bin/zkCli.sh -server 10.0.5.131:2181 ls /singularity/requests/all | tail -n1 | tr -d \"[]\" | tr -d \",\"| sed \'s/ /&\\n/g\'"
	print request
...
def main():
	...
	url = "http://app001:8082/singularity"
	for r in requests:
		print r
